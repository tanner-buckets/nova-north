...
function spotsLine(counts) {
  // One row per division when capacities are set that way, otherwise a single
  // row with division "all".
  ...
  const rows = counts.map((c) => {
    ...
    let value;
    if (c.capacity === null) {
      value = `${c.confirmed_count} registered`;
    } else {
...
function eventCard(event, counts) {
  const badges = [el('li', { className: 'badge', text: TYPE_LABELS[event.event_type] || event.event_type })];
  if (event.is_premier) {
  ...
  const body = [
    el('p', { className: 'event-when' }, [
      el('time', { datetime: event.starts_at, text: formatEventDay(event.starts_at) }),
      el('span', { className: 'event-time', text: formatEventTime(event.starts_at) })
    ]),
  ...
  if (event.registration_open) {
    ...
    body.push(el('p', { className: 'event-note', text: 'Sign-up for this event is not open yet. Just turn up, or ask a professor.' }));
  }
...
async function refresh() {
  try {
    ...
    list.replaceChildren(el('ul', { className: 'card-list' },
